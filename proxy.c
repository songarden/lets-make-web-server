...
void doit(int clientfd)
{
  ...
  if (strcasecmp(method, "GET") && strcasecmp(method, "HEAD")) //GET과 HEAD만 지원
  {
  ...
  // Server 소켓 생성
  serverfd = is_local_test ? Open_clientfd(hostname, port) : Open_clientfd("3.36.115.39", port);
  if (serverfd < 0)
  {
